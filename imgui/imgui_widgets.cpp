...
static ImVec2           InputTextCalcTextSizeW2(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL, ImVec2* out_offset = NULL, bool stop_on_new_line = false);
...
static ImVec2 InputTextCalcTextSizeW2(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ImGuiContext& g = *GImGui;
    ImFont* font = g.Font;
    const float line_height = g.FontSize;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    SetCursorPos(GetCursorPos() - ImVec2(-2, 10));
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, GetContentRegionMax().x - 14, (is_multiline ? g.FontSize * 8.0f : label_size.y + 8) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive || hovered ? ImColor(32, 32, 48) : ImColor(223, 224, 225), g.IO.DeltaTime * 6.f);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(IM_COL32(24, 24, 35, 255)); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            float bg_offy_up = is_multiline ? 0.0f : -1.0f;    // FIXME: those offsets should be part of the style? they don't play so well with multi-line selection.
            float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
            ImVec2 rect_pos = draw_pos + select_start_offset + ImVec2(11, 10);
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min - ImVec2(11, 7), rect.Max - ImVec2(11, 5), bg_color, style.FrameRounding);
                }
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? IM_COL32(223, 224, 225, 255) : IM_COL32(223, 224, 225, 255));
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(2, 3), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(it_anim->second.input_move_one + ImVec2(2, 3), it_anim->second.input_move_two + ImVec2(2, 5), GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? IM_COL32(223, 224, 225, 255) : IM_COL32(223, 224, 225, 255));
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(2, 3), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
